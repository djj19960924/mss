...
class SKTUploadOrder extends React.Component {
  ...
  queryParcelInfoToBc() {
    ...
    const data = {pageNum, pageSize, parm: BCStatus};
    this.ajax.post(`/backend/bcManagement/getBcInfoByStatus`, data).then(r => {
  ...
  showDetail(record) {
    ...
    Modal.info({
      ...
      content: <div style={hidden}>
        <div style={hidden}><div style={style}>客户内部单号: </div>{record.parcelCode}</div>
